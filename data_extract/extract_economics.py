"""
Provides functions extract daily economics data from quandl. Snap time to be confirmed.
...
def economics_master_update(file_name, update_date, all_dates, rates):
    ...
    logger.info("Extracting economics data for existing tickers")
    if len(new_dates) > 0:
        ...
        for i in range(len(rates)):
            ...
            for d in new_dates:
                try:
                    ...
                    logger.error("No economics rates available for " + name + ' on date: ' + str(d)[:10])
